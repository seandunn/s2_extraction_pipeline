define([
  ...
  return function(context) {
    ...
    var error   = _.partial(_.bind(html.trigger, html), "s2.status.error");
    var success = _.partial(_.bind(html.trigger, html), "s2.status.success");
    ...
    html.on("s2.registered", $.stopsPropagation($.ignoresEvent(function(labware) {
    ...
    html.on("s2.labware.present", $.ignoresEvent(function(labware) {
      promises[labware.resourceType].resolve(labware);
    }));
    html.on("s2.done", $.stopsPropagation($.ignoresEvent(function(view) {
  ...
  function PrintComponent(context) {
    var eventsToBindToUs = ["s2.print.labels", "s2.print.filter"];
    ...
    html.trigger("s2.print.filter", function(printer) { return printer.canPrint(context.model); });
    ...
    html.on("s2.print.trigger", $.stopsPropagation($.ignoresEvent(function(printer) {
      context.root().then(function(root) {
        Operations.registerLabware(
          ...
          // are printed.
          html.trigger("s2.registered", state.labware);
          html.trigger("s2.print.labels", [printer, [state.labware]]);
        });
      });
    })));
