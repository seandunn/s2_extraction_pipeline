define([ 'lib/underscore_extensions' ], function() {
  ...
  return {
      parse: function(obj, total_volume) {
          return header(obj) + dynMappings(obj) + "C;\n" + buffers(obj, total_volume) + footer(obj);
      },
      to : function(userName, date, requests) {
          return objToTecanFile(_.chain(requests).filter(function(request) {
              ...
              if (_.isUndefined(obj.destination[plateDst.ean13_barcode].mapping)) {
                  obj.destination[plateDst.ean13_barcode].mapping = [];
              }
              obj.destination[plateDst.ean13_barcode].mapping.push({
                  "src_well" : [
